# LScasino

LScasino is a lightweight, browser-based casino game that runs locally on your machine. It simulates a simple casino environment with classic games, allowing you to play, test mechanics, or experiment with game logic without needing a backend server or online deployment.

Features:
-Runs entirely on localhost

-Simple and responsive web interface

-Fast setup (no database required)

-Classic casino gameplay (sadly only bj for now)

-Lightweight and beginner-friendly

-Easy to modify and extend with new games or features

-Fast updates with no EOL

Tech Stack

HTML

CSS

JavaScript

(You can add any database you want or simply make a request in the issues section)
